...
module top_soc (
    ...
    // Interface: Caches <-> CDC Bridges (Miền CPU)
    wire        ic_cdc_req, ic_cdc_ready, ic_cdc_resp_val;
    wire [31:0] ic_cdc_addr, ic_cdc_rdata;
    wire        dc_cdc_rd, dc_cdc_wr, dc_cdc_ready, dc_cdc_resp_val;
    wire [31:0] dc_cdc_addr, dc_cdc_wdata, dc_cdc_rdata;
    wire [1:0]  dc_cdc_size;

    // Interface: CDC Bridges <-> Adapters (Miền Bus)
    wire        ic_bus_req, ic_bus_ready, ic_bus_resp_val;
    wire [31:0] ic_bus_addr, ic_bus_rdata;
    ...
    instruction_cache I_CACHE (
        ...
        .mem_read_req(ic_cdc_req), .mem_addr(ic_cdc_addr), .mem_read_data({32'b0, ic_cdc_rdata}), // Nối bus 64-bit [cite: 449, 456]
        .mem_read_ready(ic_cdc_ready), .mem_read_valid(ic_cdc_resp_val)
    );
    ...
    data_cache D_CACHE (
        ...
        .mem_read_ready(dc_cdc_ready), .mem_read_valid(dc_cdc_resp_val), .mem_write_ready(dc_cdc_ready), .mem_write_back_valid(dc_cdc_resp_val) // Handshake [cite: 339, 340]
    );

    // =========================================================================
    // 5. CDC BRIDGES (Miền CPU <-> Miền Bus)
    // =========================================================================
    native_cdc_bridge I_CDC_BRG (
        .cpu_clk(clk_sys), .cpu_rst_n(sys_rst_n),
        .cpu_req_val(ic_cdc_req), .cpu_req_is_write(1'b0), .cpu_req_addr(ic_cdc_addr), .cpu_req_wdata(32'd0), .cpu_req_wstrb(4'b0000), .cpu_req_size(2'b10), .cpu_req_ready(ic_cdc_ready),
        .cpu_resp_val(ic_cdc_resp_val), .cpu_resp_rdata(ic_cdc_rdata),
        .bus_clk(clk_sys), .bus_rst_n(sys_rst_n),
        .bus_req_val(ic_bus_req), .bus_req_is_write(), .bus_req_addr(ic_bus_addr), .bus_req_wdata(), .bus_req_wstrb(), .bus_req_size(), .bus_req_ready(ic_bus_ready),
        .bus_resp_val(ic_bus_resp_val), .bus_resp_rdata(ic_bus_rdata)
    );
    ...
    axi_master_adapter M0_ICACHE_ADAPT (
        .clk(clk_sys), .rst_n(sys_rst_n),
        .cpu_read_req(ic_bus_req), .cpu_write_req(1'b0), .cpu_addr(ic_bus_addr), .cpu_wdata(32'b0), .cpu_mem_size(2'b10),
        .cpu_rdata(ic_bus_rdata), .cpu_ready(ic_bus_ready), .cpu_resp_val(ic_bus_resp_val),
    ...
    axi_master_adapter M1_DCACHE_ADAPT (
        ...
        .cpu_rdata(dc_bus_rdata), .cpu_ready(dc_bus_ready), .cpu_resp_val(dc_bus_resp_val),
    ...
    // =========================================================================
    // 8. APB SUBSYSTEM (Slave 2: Bridge)
    // =========================================================================
    ...
    apb_gpio  GPIO_INST  (.pclk(clk_gpo_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_gpio),  .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pready(ready_gpio), .prdata(rdata_gpio), .pslverr(err_gpio), .gpio_in(gpio_in), .gpio_out(gpio_out), .gpio_dir(gpio_dir), .gpio_irq(irq_gpio));
    apb_accelerator ACCEL_INST (.pclk(clk_acc_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_accel), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_accel), .prdata(rdata_accel), .pslverr(err_accel), .accel_irq(irq_accel));
