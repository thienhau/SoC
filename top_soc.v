...
module top_soc (
    ...
    input  wire        clk_sys,        // Tần số hoạt động hệ thống (300MHz - 400MHz)
    input  wire        rst_n_pad,      // Tín hiệu Reset cứng từ chân vật lý (Active Low)
    ...
    // --- Giao tiếp AXI SPI Flash (Firmware Storage) ---
    ...
    // =========================================================================
    // 1. RESET SYNCHRONIZER (Đồng bộ hóa tín hiệu Reset hệ thống)
    // =========================================================================
    reg [2:0] rst_sync;
    ...
    // =========================================================================
    // 2. KHAI BÁO CÁC ĐƯỜNG TÍN HIỆU KẾT NỐI (INTERNAL WIRES)
    // =========================================================================
    wire [31:0] sys_reset_vector;
    wire        plic_ext_irq;
    wire        cpu_wfi_sleep;
    
    // Tín hiệu giữa CPU Core và Caches
    ...
    // Tín hiệu JTAG Internal
    wire        j_shift, j_update, j_capture, j_sel_dmi, j_tdo_dmi;
    wire [4:0]  j_ir;

    // Tín hiệu giữa Caches và CDC Bridge
    ...
    // Tín hiệu từ CDC Bridge ra Bus Adapter
    ...
    // =========================================================================
    // 3. CLOCK GATING UNIT (Quản lý năng lượng ngoại vi)
    // =========================================================================
    ...
    // =========================================================================
    // 4. JTAG VÀ DEBUG MODULE (Hỗ trợ Debug GDB/OpenOCD)
    // =========================================================================
    ...
    // =========================================================================
    // 5. KHỐI CPU CORE VÀ HỆ THỐNG CACHE (ĐÃ SỬA HANDSHAKE)
    // =========================================================================
    ...
    instruction_cache I_CACHE (
        ...
        .rst_n(sys_rst_n),
        .flush(cpu_flush),
        .cpu_read_req(cpu_ic_req),
        .cpu_addr(cpu_ic_addr),
        .cpu_read_data(cpu_ic_rdata),
        .icache_stall(cpu_ic_stall),
        .icache_hit(cpu_ic_hit),
        .mem_read_req(ic_cdc_req),
        .mem_addr(ic_cdc_addr),
        .mem_read_data({32'b0, ic_cdc_rdata}),
        .mem_read_ready(ic_cdc_ready),    // Kết nối mới cho cơ chế bắt tay
        .mem_read_valid(ic_cdc_resp_val)
    );

    data_cache D_CACHE (
        .clk(clk_sys),
        .rst_n(sys_rst_n),
        .cpu_read_req(cpu_dc_rd),
        .cpu_write_req(cpu_dc_wr),
        .cpu_addr(cpu_dc_addr),
        .cpu_write_data(cpu_dc_wdata), 
        .cpu_read_data(cpu_dc_rdata),
        .dcache_stall(cpu_dc_stall),
        .dcache_hit(cpu_dc_hit),
        .mem_unsigned(cpu_mem_unsigned),
        .mem_size(cpu_dc_size),
        .mem_read_req(dc_cdc_rd),
        .mem_write_req(dc_cdc_wr),
        .mem_addr(dc_cdc_addr), 
        .mem_write_data(dc_cdc_wdata),
        .mem_read_data(dc_cdc_rdata),
        .mem_read_ready(dc_cdc_ready),   // Kết nối mới cho kênh đọc
        .mem_write_ready(dc_cdc_ready),  // Kết nối mới cho kênh ghi
        .mem_read_valid(dc_cdc_resp_val),
        .mem_write_back_valid(dc_cdc_resp_val)
    );

    // =========================================================================
    // 6. MIỀN BUS AXI (INTERCONNECT VÀ ADAPTERS)
    // =========================================================================
    ...
    // Adapters chuyển đổi từ Native Cache interface sang AXI4-Lite
    axi_master_adapter M0_ICACHE_ADAPT (
        .clk(clk_sys), .rst_n(sys_rst_n),
        .cpu_read_req(ic_cdc_req), .cpu_write_req(1'b0), .cpu_addr(ic_cdc_addr), .cpu_wdata(32'b0), .cpu_mem_size(2'b10),
        .cpu_rdata(ic_cdc_rdata), .cpu_ready(ic_cdc_ready),
    ...
    axi_master_adapter M1_DCACHE_ADAPT (
        .clk(clk_sys), .rst_n(sys_rst_n),
        .cpu_read_req(dc_cdc_rd), .cpu_write_req(dc_cdc_wr), .cpu_addr(dc_cdc_addr), .cpu_wdata(dc_cdc_wdata), .cpu_mem_size(cpu_dc_size),
        .cpu_rdata(dc_cdc_rdata), .cpu_ready(dc_cdc_ready),
    ...
    // =========================================================================
    // 7. AXI INTERCONNECT VÀ CÁC SLAVES (RAM, ROM, SPI FLASH, APB BRIDGE)
    // =========================================================================
    wire [31:0] s0_araddr, s1_awaddr, s1_araddr, s2_awaddr, s2_araddr, s3_araddr;
    ...
    wire [31:0] s0_rdata, s1_wdata, s1_rdata, s2_wdata, s2_rdata, s3_rdata;
    wire [3:0]  s1_wstrb, s2_wstrb;
    wire [1:0]  s0_rresp, s1_bresp, s1_rresp, s2_bresp, s2_rresp, s3_rresp;
    ...
    axi_ram SYSTEM_RAM_INST (
        ...
        .s_axi_rdata(s1_rdata), .s_axi_rresp(s1_rresp), .s_axi_rvalid(s1_rvalid), .s_axi_rready(s1_rready)
    );

    axi_spi_flash FLASH_CTRL_INST (
        .aclk(clk_sys), .aresetn(sys_rst_n),
        .araddr(s3_araddr), .arvalid(s3_arvalid), .arready(s3_arready),
        .rdata(s3_rdata), .rresp(s3_rresp), .rvalid(s3_rvalid), .rready(s3_rready),
        .spi_cs_n(spi_flash_cs_n), .spi_sck(spi_flash_sck), .spi_mosi(spi_flash_mosi), .spi_miso(spi_flash_miso)
    );

    // =========================================================================
    // 8. HỆ THỐNG NGOẠI VI APB (PERIPHERAL SUBSYSTEM)
    // =========================================================================
    wire [31:0] apb_paddr, apb_pwdata, apb_prdata;
    wire [3:0]  apb_pstrb;
    wire        apb_psel, apb_penable, apb_pwrite, apb_pready, apb_pslverr;

    axi_to_apb_bridge #(.ADDR_WIDTH(32)) BRIDGE_INST (
        .clk(clk_sys), .rst_n(sys_rst_n),
    ...
    // =========================================================================
    // 9. INSTANTIATE CÁC NGOẠI VI (PERIPHERAL INSTANCES)
    // =========================================================================
    wire irq_timer, irq_uart, irq_spi, irq_i2c, irq_gpio, irq_accel;

    apb_syscon SYSCON_INST (
        .pclk(clk_sys), .presetn(sys_rst_n), .paddr(apb_paddr[15:0]), .psel(sel_syscon), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pready(ready_syscon), .prdata(rdata_syscon), .pslverr(err_syscon), 
        .o_reset_vector(sys_reset_vector), .i_wfi_sleep(cpu_wfi_sleep), .i_ext_irq(plic_ext_irq), 
        .o_cpu_clk_en(clk_en_cpu), .o_dbg_clk_en(clk_en_dbg), .o_tmr_clk_en(clk_en_tmr), .o_urt_clk_en(clk_en_urt), .o_spi_clk_en(clk_en_spi), .o_i2c_clk_en(clk_en_i2c), .o_gpo_clk_en(clk_en_gpo), .o_acc_clk_en(clk_en_acc)
    );

    apb_interrupt_controller PLIC_INST (
        .pclk(clk_sys), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_plic), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_plic), .prdata(rdata_plic), .pslverr(err_plic), 
        .irq_timer(irq_timer), .irq_uart(irq_uart), .irq_spi(irq_spi), .irq_i2c(irq_i2c), .irq_gpio(irq_gpio), .irq_accel(irq_accel), .cpu_ext_irq(plic_ext_irq)
    );

    apb_timer TIMER_INST (
        .pclk(clk_tmr_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_timer), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_timer), .prdata(rdata_timer), .pslverr(err_timer), 
        .timer_irq(irq_timer)
    );

    apb_uart UART_INST (
        .pclk(clk_urt_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_uart), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_uart), .prdata(rdata_uart), .pslverr(err_uart), 
        .rx(uart_rx), .tx(uart_tx), .uart_irq(irq_uart)
    );

    apb_spi SPI_INST (
        .pclk(clk_spi_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_spi), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_spi), .prdata(rdata_spi), .pslverr(err_spi), 
        .sclk(spi_sclk), .mosi(spi_mosi), .miso(spi_miso), .cs_n(spi_cs_n), .spi_irq(irq_spi)
    );

    apb_i2c I2C_INST (
        .pclk(clk_i2c_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_i2c), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_i2c), .prdata(rdata_i2c), .pslverr(err_i2c), 
        .scl_o(i2c_scl_o), .scl_oen(i2c_scl_oen), .scl_i(i2c_scl_i), .sda_o(i2c_sda_o), .sda_oen(i2c_sda_oen), .sda_i(i2c_sda_i), .i2c_irq(irq_i2c)
    );

    apb_gpio GPIO_INST (
        .pclk(clk_gpo_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_gpio), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pready(ready_gpio), .prdata(rdata_gpio), .pslverr(err_gpio), 
        .gpio_in(gpio_in), .gpio_out(gpio_out), .gpio_dir(gpio_dir), .gpio_irq(irq_gpio)
    );

    apb_accelerator ACCEL_INST (
        .pclk(clk_acc_gated), .presetn(sys_rst_n), .paddr(apb_paddr[11:0]), .psel(sel_accel), .penable(apb_penable), .pwrite(apb_pwrite), .pwdata(apb_pwdata), .pstrb(apb_pstrb), .pready(ready_accel), .prdata(rdata_accel), .pslverr(err_accel), 
        .accel_irq(irq_accel)
    );
