// =============================================================================
// SUB-MODULE: DCache Tag RAM (Lưu trữ 4-Way Tags) - 16 Sets x 6-bit Tag
// =============================================================================
module dcache_tag_ram (
    input clk,
    input [3:0] index,
    input [3:0] write_en, // One-hot: [0]=Way1 ... [3]=Way4
    input [5:0] write_tag,
    output [5:0] t1, t2, t3, t4
);
    (* ram_style = "distributed" *) reg [5:0] tag1 [0:15];
    (* ram_style = "distributed" *) reg [5:0] tag2 [0:15];
    (* ram_style = "distributed" *) reg [5:0] tag3 [0:15];
    (* ram_style = "distributed" *) reg [5:0] tag4 [0:15];
...
// =============================================================================
// SUB-MODULE: DCache Data RAM (Lưu trữ 4-Way Data) - 16 Sets x 32-bit Data
// =============================================================================
...
// =============================================================================
// MAIN MODULE: Data Cache Controller (12-bit Address Logic)
// =============================================================================
module data_cache (
    input clk, reset,
    input cpu_read_req, input cpu_write_req,
    input [11:0] cpu_addr,
    input [31:0] mem_read_data,
    input [31:0] cpu_write_data,
    input mem_unsigned, input [1:0] mem_size,
    input mem_read_valid, input mem_write_back_valid,
    output reg mem_read_req, output reg mem_write_req,
    output reg [11:0] mem_addr,
    output reg [31:0] cpu_read_data,
    output reg [31:0] mem_write_data,
    output reg dcache_hit, output reg dcache_stall
);
        ...
        begin
            case (size)
                ...
                2'b01: begin // Half-word
                    if (offset[1] == 0) result = unsigned_flag ? {16'b0, data[15:0]} : {{16{data[15]}}, data[15:0]};
    ...
    function [31:0] write_data_with_size;
        ...
        begin
            ...
            case (size)
                ...
                2'b01: begin // Half-word
                    if (offset[1] == 0) result[15:0]  = write_data[15:0];
    ...
    function [2:0] update_plru;
        input [2:0] old_plru; input [1:0] accessed_way;
        reg [2:0] new_plru;
        begin
            new_plru = old_plru;
            case (accessed_way)
    ...
    // --- PHÂN TÍCH ĐỊA CHỈ VÀ BIẾN NỘI BỘ ---
    wire [5:0] tag = cpu_addr[11:6];
    wire [3:0] index = cpu_addr[5:2];
    wire [1:0] byte_offset = cpu_addr[1:0];

    wire [5:0] t1, t2, t3, t4;
    ...
    // Kết nối Sub-Modules
    dcache_tag_ram TAGS (
        .clk(clk), .index(index), .write_en(way_write_en), 
        .write_tag(tag), .t1(t1), .t2(t2), .t3(t3), .t4(t4)
    );

    dcache_data_ram DATA (
        .clk(clk), .index(index), .write_en(way_write_en), 
        .write_data(final_write_data), .d1(d1), .d2(d2), .d3(d3), .d4(d4)
    );
    ...
    always @(posedge clk) begin
        if (reset) begin
            ...
            for(i=0; i<16; i=i+1) begin 
                valid1[i]<=0; dirty1[i]<=0; valid2[i]<=0; dirty2[i]<=0; 
                valid3[i]<=0; dirty3[i]<=0; valid4[i]<=0; dirty4[i]<=0; 
                plru[i]<=0; 
            end
        end else begin
            state <= next_state;
            ...
            if (way_write_en != 4'b0000) begin
                ...
                if (state == MEM_READ) begin 
                    ...
                    case(target_way)
                        0: begin valid1[index]<=1; dirty1[index]<=0; end
                        1: begin valid2[index]<=1; dirty2[index]<=0; end
                        2: begin valid3[index]<=1; dirty3[index]<=0; end
                        3: begin valid4[index]<=1; dirty4[index]<=0; end
                    endcase
                end else if (state == IDLE) begin 
                    // Cập nhật từ CPU (Write Hit hoặc Write Miss Allocate)
                    case(target_way)
                        0: begin valid1[index]<=1; dirty1[index]<=1; end
                        1: begin valid2[index]<=1; dirty2[index]<=1; end
                        2: begin valid3[index]<=1; dirty3[index]<=1; end
                        3: begin valid4[index]<=1; dirty4[index]<=1; end
                    endcase
                end
    ...
    always @(*) begin
        // Reset ngõ ra
        next_state = state;
        dcache_hit = 0; dcache_stall = 0;
        mem_read_req = 0; mem_write_req = 0; mem_addr = 0;
        cpu_read_data = 0; mem_write_data = 0;
        way_write_en = 0; final_write_data = 0; victim_dirty = 0;

        // 1. Hit/Miss Detection & Victim Selection
        if      (valid1[index] && t1 == tag) begin dcache_hit=1; target_way=0; end
        else if (valid2[index] && t2 == tag) begin dcache_hit=1; target_way=1; end
        else if (valid3[index] && t3 == tag) begin dcache_hit=1; target_way=2; end
        else if (valid4[index] && t4 == tag) begin dcache_hit=1; target_way=3; end
        else begin
            dcache_hit = 0;
            // Thuật toán ưu tiên invalid way trước, sau đó dùng PLRU
            if      (!valid1[index]) target_way = 0;
            else if (!valid2[index]) target_way = 1;
            else if (!valid3[index]) target_way = 2;
            else if (!valid4[index]) target_way = 3;
            else target_way = select_replacement_way(plru[index]);
        end

        // 2. Kiểm tra Victim có Dirty không?
        case (target_way)
            0: victim_dirty = dirty1[index] & valid1[index];
            1: victim_dirty = dirty2[index] & valid2[index];
            2: victim_dirty = dirty3[index] & valid3[index];
            3: victim_dirty = dirty4[index] & valid4[index];
        endcase
        ...
        case (state)
            IDLE: begin
                if (cpu_read_req) begin
                    if (dcache_hit) begin
                        // --- READ HIT ---
                        case (target_way)
                            0: cpu_read_data = read_data_with_size(d1, mem_size, byte_offset, mem_unsigned);
                            1: cpu_read_data = read_data_with_size(d2, mem_size, byte_offset, mem_unsigned);
                            2: cpu_read_data = read_data_with_size(d3, mem_size, byte_offset, mem_unsigned);
                            3: cpu_read_data = read_data_with_size(d4, mem_size, byte_offset, mem_unsigned);
                        endcase
                    end else begin
                        // --- READ MISS ---
                        dcache_stall = 1;
                    ...
                    if (dcache_hit) begin
                        // --- WRITE HIT ---
                        way_write_en[target_way] = 1; // Kích hoạt ghi RAM
                        case (target_way)
                            0: final_write_data = write_data_with_size(d1, cpu_write_data, mem_size, byte_offset);
                            1: final_write_data = write_data_with_size(d2, cpu_write_data, mem_size, byte_offset);
                            2: final_write_data = write_data_with_size(d3, cpu_write_data, mem_size, byte_offset);
                            3: final_write_data = write_data_with_size(d4, cpu_write_data, mem_size, byte_offset);
                        endcase
                    end else begin
                        // --- WRITE MISS ---
                        if (victim_dirty) begin
                            ...
                            dcache_stall = 0; // Không bị trễ
                            way_write_en[target_way] = 1;
            ...
            MEM_READ: begin
                dcache_stall = 1;
                mem_read_req = 1;
                mem_addr = {tag, index, 2'b00}; // Load Block đang cần
            ...
            MEM_WRITE_BACK: begin
                ...
                // Chuẩn bị địa chỉ & dữ liệu cũ để đẩy ra bộ nhớ
                case (target_way)
                    0: begin mem_addr = {t1, index, 2'b00}; mem_write_data = d1; end
                    1: begin mem_addr = {t2, index, 2'b00}; mem_write_data = d2; end
                    2: begin mem_addr = {t3, index, 2'b00}; mem_write_data = d3; end
                    3: begin mem_addr = {t4, index, 2'b00}; mem_write_data = d4; end
                endcase
